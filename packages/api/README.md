...
传输使用 ofetch 实例，直接返回 `request<ResponseType>(url, { body, query })`，需要 HTTP 元信息时使用 `request.raw()`。约定见 [request 包](../request/README.md)。API 不判断业务 code、不提取 `data`；调用方业务层（例如 Landing 的 store action）按业务契约判断结果、校验和整理数据。响应可能是 undefined 或宽松解析后的文本，泛型不代替运行时校验。HTTP 和发送阶段的网络/取消错误使用 FetchError；其他阶段的异常可能是原始 Error。业务 code 不放入通用传输层。
