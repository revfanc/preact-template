...
const unregister: Unregister = register(
  async (done) => {
    ...
    // done 成功完成时停在当前页的基础记录 a，不再退到上一页。
  },
...
- 返回只执行栈顶。栈顶调用 `done()` 就放行本次，不向下层传递；回调正常完成但没有调用 `done()` 则保留在 `a+`。
- 每个注销函数只移除自己的注册。移除中间层不影响栈顶；移除栈顶后下层接任，不在同一次返回中立即调用下层。
- 回调、恢复或放行忙碌期间，不重复分发回调。新增注册或移除正在执行的注册会使旧的放行权限失效。
- `done()` 成功后注册仍保留，停在 `a` 时不重新补保护。前进到已有 `a+` 后再次返回可重新触发；在 `a` 新增注册会建立新一轮保护。
...
- 最后一层清理尚未完成时，新注册会明确报错；先 `await unregister()` 再注册。组件卸载时可以发起注销，但紧随其后的主动导航或重新注册必须等待这个 Promise，不能只依赖卸载钩子的同步返回。
...
- 回调报错会撤销尚未完成的放行权限。已经完成的放行不会撤销；已经发出的 `go()` 没有取消接口，即使权限失效，浏览器仍可能完成该次移动。
