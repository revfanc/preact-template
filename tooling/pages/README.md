...
Landing 将声明的路径交给官方预渲染器，根路径由应用入口输出空正文，生成 SPA 回退用的 `index.html`，不要求存在首页路由。Agreement 预渲染全部静态页面，无需逐页声明。兜底不生成静态错误页。

懒加载模式下，插件在预渲染后使用构建产物映射，为已生成的页面 HTML 添加当前页面及静态依赖的 `modulepreload` 和样式表。不会追踪动态导入或预加载其他路由；已有入口资源不重复添加。Landing 随后的关键 CSS 处理负责内联和异步加载外部样式。同步模式不额外处理资源。
