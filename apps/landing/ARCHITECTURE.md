...
src/
  ...
  styles/                 全局样式与应用主题
    ...
    theme.css             公共主题变量覆盖
  lib/                    应用基础能力和工具
    request.ts            应用请求实例、统一响应协议与错误
    request.test.ts       就近测试
  api/
...
`lib/request.ts` 通过 `@packages/request` 创建请求实例，浏览器适配由包内部完成。Node 预渲染允许导入和创建实例，实际调用请求会报错；store 工厂只初始化状态，请求从客户端 effect/事件触发。预渲染构建测试覆盖“页面 → store → 真实请求入口”的导入链。共享业务接口放在 `packages/api`，应用独有接口按需放入 `src/api/`。

## 状态归属

业务、表单、请求结果、pending/error 等状态统一进入所属作用域的 store。hook 和 UI 读取 store，不重复保存业务状态。
...
需要持久化时由业务 store 的客户端初始化 action 接入 `stores/core/persist.ts` 的 `persistStore`，传入固定业务 key、版本、字段白名单、同步 storage getter 和恢复校验器；有效期可选。`createStore` 保持内存容器，持久化扩展在调用时同步恢复选中字段并订阅变化。参与预渲染的 store 工厂不直接恢复缓存；所有者在 effect 中启动，恢复完成前不允许编辑或发起依赖草稿的请求，存储失败仍可使用内存。销毁停止同步与清缓存是两个动作，由实例所有者协调；具体契约与示例见 [Stores 持久化说明](src/stores/README.md#可选持久化)。
...
- 应用请求实例（`src/lib/request.ts`）：检查统一响应外层与成功码，转换业务错误，保留完整响应；不处理具体数据字段、不展示反馈。公共请求包只负责通用传输。
- Store：管理状态及数据 action，调用 API，校验和整理具体数据、判断业务状态，保存 pending/error 和结果。不得直接展示 Toast、Loading、Modal 或执行导航。
...
Landing 使用预渲染 + hydration + SPA。工厂创建稳定初始状态，客户端 effect 启动业务，所属实例清理资源；预渲染路径和部署规则见 [Landing README](README.md#预渲染与-spa)。首次渲染、渠道数据、缓存恢复、CSS 与业务验收统一遵循 [预渲染业务约定](README.md#业务开发约定)。
